...
class DQN_Algo():
    ...
    def __init__(self, filepath, lr, expl_slope, discount_factor, mem_size, batch_size, n_epochs, tau, n_timesteps, sensor="plate", global_step=None):
        ...
        if not os.path.exists(self.FILEPATH):
            ...
            with open(self.FILEPATH + '/training_state.pickle', 'rb') as f:
                training_state = pickle.load(f)
            
            self.EPOCHS_2GO = training_state[0]
            self.stepcount  = training_state[1]

            self.gapsize = training_state[2]
            self.angle_range = training_state[3]
    ...
    def save_checkpoint(self, save_buffer=False):
        ...
        if save_buffer:
            ...
            training_state = [self.EPOCHS_2GO, self.stepcount, self.gapsize, self.angle_range]
            with open(self.FILEPATH + 'training_state.pickle', 'wb') as f:
    ...
    def train(self):
        ...
        while self.EPOCHS_2GO >= 0:
            
            episode = (self.N_EPOCHS - self.EPOCHS_2GO) +1
            obs, info = self._reset_env(options={'gap_size' : self.gapsize, 'testing' : False, 'angle_range' : self.angle_range})
            obs = self._normalize_observation(obs)
...
if __name__=='__main__':
    ...
    if continue_ == 0:
        ...
        if opt.savedir is None:
            root = '/homes/mjimenezhaertel/Masterarbeit/Training/'
        else:
            root = opt.savedir 

        subdirs  = [f.path for f in os.scandir(root) if f.is_dir()]
        filepath = max(subdirs, key=os.path.getctime)
