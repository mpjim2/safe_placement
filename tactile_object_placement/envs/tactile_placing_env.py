...
import gym
from gym.spaces import Box, Discrete, Dict, Sequence
...
class TactileObjectPlacementEnv_v2(gym.Env):
    ...
    def __init__(self, object_params=dict(), curriculum=False, sensor="fingertip", continuous=False, grid_size=4, action_space='full', timesteps=10):
        ...
        self.current_msg = {"myrmex_l" : deque([np.zeros(self.num_taxels) for _ in range(self.timesteps)], maxlen=self.timesteps), 
                            "myrmex_r" : deque([np.zeros(self.num_taxels) for _ in range(self.timesteps)], maxlen=self.timesteps), 
        ...
        #Min and Max values for normalizing from "outside"
        
        self.spaces = {"ee_pose" : Box( low=np.array([-0.5, -0.5, 0, -1, -1, -1, -1]), 
        ...
        self.observation_space = Dict({
            "observation" : Dict({
                "ee_pose" : Sequence(self.spaces["ee_pose"]),
                
                "joint_positions" : Sequence(self.spaces["joint_positions"]),
                
                "joint_torques" : Sequence(self.spaces["joint_torques"]),
                
                "joint_velocities" : Sequence(self.spaces["joint_velocities"]),
        ...
        self.range_obj_height = object_params.get("range_height", np.array([0.08/2, 0.12/2])) # [m]
        ...
        self.range_obj_mass = object_params.get("range_mass", np.array([0.05, 0.1])) # [kg]
    ...
    def _set_EE_frame_to_gripcenter(self): 
        ...
        #     if sum(self.current_msg['ee_pose'][-1]) != 0:
        #         break
    ...
    def _get_obs(self, return_quat=False, initial_obs=False):
        ...
        observation = {
                    ...
                    # "contact" : int(self.current_msg['object_contact_GT'].value > 0)
                    #    "time_diff" : time_diff
                    }
    ...
    def _compute_twist(self, rotate_X, rotate_Y, translate_Z):
        ...
        noop = True
        current_pose = self.current_msg['ee_pose'][-1]
    ...
    def step(self, action):
        ...
        if action[-1] == 1:
            ...
            reward, lost, info = self._compute_reward(compute_final_reward=True)
            ...
            reward, lost, info = self._compute_reward()
        ...
        if reward != 0 or lost:
            ...
            if action[-1] == 1:
                ...
                observation = self._get_obs()
                if reward > 0:
                    stable, _, _ = self._compute_reward(compute_final_reward=True)

                    if stable > reward or stable <= 0.9999:
                        if reward >= 0.9999:
                            reward = stable
                        else:
                            reward /= 3
    ...
    def _franka_state_callback(self, msg):
        ...
        self.EE_pos, _, _ = self._pose_quat_from_trafo(msg.F_T_EE)
        pose = np.concatenate([ee_pos, ee_quat])

        if self.pitch_only:
            euler = quaternion_to_euler(ee_quat)
            pose = euler[1]
    ...
    def _new_msg_callback(self, msg):
        ...
        if not key:
            ...
            if key[0] == 'myrmex_r' or key[0] == 'myrmex_l':
                ...
                myrmex_data = np.array(msg.sensors[0].values)
                
                if self.sensor == 'fingertip':
                    myrmex_data = np.array([np.max(myrmex_data[i*5:(i+1)*5]) for i in range(int(len(myrmex_data)/5))]) / self.myrmex_max_val#+ 0.0000001*np.random.randn(self.num_taxels)
