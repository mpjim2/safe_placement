...
class TactileObjectPlacementEnv(gym.Env):
    ...
    def _get_obs(self, return_quat=False, initial_obs=False):
        ...
        while not valid_obs:
            ...
            # myrmex_data_l = np.array(self.current_msg["myrmex_l"].sensors[0].values)
            ...
            # myrmex_data_r = np.array(self.current_msg["myrmex_r"].sensors[0].values)
    ...
    def _compute_twist(self, rotate_X, rotate_Y, translate_Z):
        ...
        noop = True
        current_pose = self._get_obs(return_quat=False)
